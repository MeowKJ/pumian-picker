...
选择输出目录后，点击顶部 `iPad 传输`，或在右侧切到 `iPad 传输`：

1. 点击 `打包并开启扫码下载`。
2. 应用会只打包完整歌曲文件夹。
3. ZIP 内顶层文件夹固定为 `levels`，歌曲文件夹会直接放在里面，例如 `levels/歌曲文件夹/...`。
4. 传输页会显示本机局域网 IPv4 二维码和下载地址。
5. iPad 与电脑连到同一局域网后，用相机或 Safari 扫码下载 ZIP。
6. 下载完成后在 iPad 文件 App 中解压。
...
局域网扫码下载需要电脑和 iPad 处在同一个网络中。应用会优先选择本机 Wi-Fi/有线网卡的私有 IPv4，避免使用虚拟网卡地址。如果二维码无法访问，通常是网络隔离、防火墙或设备不在同一局域网导致。
